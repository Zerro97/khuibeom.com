...
<template>
  <div class="flex justify-between w-full h-16 md:h-20 backdrop-blur-md bg-zinc-900">
    <div class="flex items-center justify-center h-full w-max">
      <NuxtLink to="/" data-cy="link-home" class="transition p-1 mr-2 rounded sm:p-2 sm:mr-4 md:mr-8 hover:bg-zinc-800" aria-label="Link to home">
      ...
      <div class="flex items-center h-full gap-x-3 sm:gap-x-5 md:gap-x-8">
        <NuxtLink
          ...
          :data-cy="tab.cypress"
          :to="tab.link"
          :class="`${isMatching(tab.regex) ? 'text-violet-400' : 'text-zinc-300 hover:text-zinc-100'} transition text-sm sm:text-base  md:text-lg border-violet-500`"
        >
          {{ tab.label }}
        </NuxtLink>
      </div>

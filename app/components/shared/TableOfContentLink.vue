  ...
  <ul class="list-none">
    <li v-for="link in links" :key="link.text" class="relative">
      <template v-if="link.depth < 5">
        <div :class="`${activeHeadings.includes(link.id) ? 'bg-primary-400' : 'bg-neutral-700'} transition absolute left-0 top-[0.35rem] h-5 w-[3px] rounded`" />
        <a
          :href="`#${link.id}`"
          :class="`depth-${link.depth} ${activeHeadings.includes(link.id) ? 'text-primary-300' : 'text-neutral-500'} text-sm`"
